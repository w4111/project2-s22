# -*- coding: utf-8 -*-
"""Copy of project2_sol.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/17P_tM_Vi96GEN8BcxiTkczmrjkfF8Cc_
...
For this assignment, you will do a graph analysis to analyze Tweets during the 2018 election.
...
### Step 1. Setup DuckDB
* Don't modify the following setup codes! Don't change the db variable!
"""

# !pip install duckdb==0.3.1
# !rm tweets.duckdb*
# !wget "https://www.dropbox.com/s/ayjsytfg9t152h6/tweets.duckdb"

import duckdb
db = duckdb.connect('tweets.duckdb')

def runq(q, qdb=db):
  "I'm helper function to run and print queries"
  cursor = qdb.execute(q)
  df = cursor.fetchdf()
  return df
...
* "The New York Times **Maga**zine" is a match
...
Hint: take a look at the [DuckDB documentation for text functions](https://duckdb.org/docs/sql/functions/char).

### Write your SQL below
"""

q1 = """
SELECT id, text FROM tweets WHERE contains(lcase(text), 'maga') and contains(lcase(text), 'trump') 
"""

result = runq(q1)
assert len(result.columns)== 2, "The result should contain two columns: id and text!"
result
...
Let's find out Top 5 months that get the maximum mention of this slogan
"MAGA"(**case-insensitive**) in `tweet_text`. 
...
Hint: look at the documentation for [parsing strings into dates](https://duckdb.org/docs/sql/functions/dateformat) and the [date functions](https://duckdb.org/docs/sql/functions/date).
...
q2 = """
WITH tmp AS (
    SELECT  strptime(create_time, '%a %b %-d %H:%M:%S +0000 %Y') as dt FROM tweets
    WHERE contains(lcase(text), 'maga')
)
...
result = runq(q2)
assert len(result.columns)== 3, "The result should contain three columns: year, month and count!"
result

"""## Question 3   
**Let's construct the Graph!**

Twitter lets users mention another user in the tweet using the "@username" notation.   (For the purposes of this project) the syntax for a mention is the character `@` followed by one or more alphanumeric characters.  The username ends with either a non alphanumeric character or end of string.

For example, the string `".a@eugene123's"` mentions `eugene123`.
The string `"@TIME a"` mentions `TIME`.
...
Hint: look at DuckDB's [regular expression pattern matching](https://duckdb.org/docs/sql/functions/patternmatching) as well as the [text functions](https://duckdb.org/docs/sql/functions/char).

---


To drop and create a table, the syntax are:
...
with tmp as (select twitter_username as src, regexp_extract(text, '@[a-zA-Z0-9_]+') as dst from tweets)
...
# test if the created table is readable
test_graph = "SELECT * FROM Graph where prefix(src, '@');"

result = runq(test_graph)
assert len(result.columns)== 2, "The result should contain two columns: src, dst!"
result
...
q4 = """
SELECT (SELECT dst FROM Graph GROUP BY dst ORDER BY count(*) desc LIMIT 1) as max_indegree,
       (SELECT src FROM Graph GROUP BY src ORDER BY count(*) desc LIMIT 1) as max_outdegree;
"""

result = runq(q4)
assert len(result.columns)== 2, "The result should contain two columns: max_indegree, max_outdegree!"
assert len(result)== 1, "The result should contain one row!"
result
...
Now, compute the following: given all tweets by unpopular users, what percentage of them mention a popular user?  We will only consider users in this dataset. 
...
q5 = """
-- indegrees of users in the graph (both dst and src)
CREATE OR REPLACE VIEW indegree AS (SELECT dst as user, count(*) as cnt FROM Graph GROUP BY dst) UNION (SELECT src as user, 0 as cnt FROM Graph WHERE src not in (SELECT dst from Graph) GROUP BY src);
-- avergae indegree
CREATE OR REPLACE VIEW avg_indegree AS SELECT avg(cnt) as cnt FROM indegree;
-- likes of user
CREATE OR REPLACE VIEW likes AS SELECT twitter_username as user, avg(like_num) as likes FROM tweets WHERE twitter_username in (SELECT dst FROM Graph) or twitter_username in (SELECT src FROM Graph) GROUP BY twitter_username;
-- average likes
CREATE OR REPLACE VIEW avg_likes AS SELECT avg(like_num) as likes FROM tweets;
-- unpopular user
CREATE OR REPLACE VIEW unpopular AS
...
FROM indegree, likes
WHERE indegree.user = likes.user AND indegree.cnt >= (SELECT * FROM avg_indegree) AND likes.likes >= (SELECT * FROM avg_likes);
-- unpopular user tweets
CREATE OR REPLACE VIEW unpopular_tweets AS
SELECT twitter_username as src, regexp_extract(text, '@[\w\d]+')[1:] as dst 
FROM tweets
WHERE twitter_username IN (SELECT * FROM unpopular);
-- unpopular user tweets containing popular users
CREATE OR REPLACE VIEW unpopular_tweets_popular AS
SELECT * 
FROM unpopular_tweets
WHERE dst IN (SELECT * FROM popular);
-- final result
SELECT CAST((SELECT COUNT(*) FROM unpopular_tweets_popular) AS DOUBLE)/(SELECT COUNT(*)  FROM unpopular_tweets) AS unpopular_popular;
"""


result = runq(q5)
assert len(result.columns)== 1, "The result should contain one column: unpopular_popular!"
assert len(result)== 1, "The result should contain one row!"
result
...
result = runq(q6)
assert len(result.columns)== 1, "The result should contain one column: no_of_triangles!"
assert len(result)== 1, "The result should contain one row!"
result
...
Hint: You may use iterative implementation for PageRank (like the BFS example mentioned below).   Alternatively, you may use the `WITH RECURSIVE` syntax to write a single recursive query.  See [DuckDB's WITH syntaxt documentation](https://duckdb.org/docs/sql/query_syntax/with) for details.
...
PR(A)= PR(B)/L(B) + PR(C)/L(C) + PR(D)/L(D) where L(x) gives us the number of outbound links for any node x, and 
...
**For this question, you can implement the simplified version of the PageRank algorithm. But it will be great if you handle dangling node and reducible graph. To read more about PageRank, you can refer to the following link:** [PageRank](http://ilpubs.stanford.edu:8090/422/1/1999-66.pdf)
...
def bfs(start, n_iter, db):
    ...
    create_graph = """
        ...
        INSERT INTO bfs_graph(src, dst) VALUES
        ('A', 'B'),
        ('A', 'E'),
        ('B', 'C'),
        ('C', 'D'),
    ...
    runq(create_graph, db)

    # distances stores each node and its minimum distance from A
    create_distances = """
        DROP TABLE IF EXISTS distances;
        CREATE TABLE distances AS
        SELECT '{start}' as node, 0 as distance
        """.format(start=start)
    runq(create_distances, db)

    # In each iteration, we take one step in the BFS algorithm 
    # by following edges from nodes at the current distance 
    # and adding the new nodes that have been encountered.
    for i in range(n_iter):
        print("Step %d..." % (i+1))
        update_distances = """
        INSERT INTO distances(node, distance)
        ...
        results = runq(update_distances, db)
    return runq("SELECT * FROM distances", db)
...
def page_rank(n_iter, db):
    """fill in this function"""
    
    # Initialize page rank by dropping/creating the appropriate tables
    initial_rank = """
    -- Create Nodes Table
    CREATE OR REPLACE VIEW nodes AS (SELECT DISTINCT src AS usr FROM Graph) UNION (SELECT DISTINCT dst AS usr FROM Graph);
    CREATE OR REPLACE VIEW outdegrees AS SELECT src AS usr, COUNT(*) AS out from Graph GROUP BY src;
    DROP TABLE IF EXISTS ranks_0;
    -- ranks_0 stores the initial user rank scores, and their out-degree
    CREATE TABLE ranks_0 AS
    SELECT DISTINCT src as usr, 1/CAST((SELECT COUNT(*) FROM nodes) AS Double) AS Rank
    FROM Graph;
    """
    runq(initial_rank, db)

    for i in range(n_iter):
        # Make one iteration of page rank
        iteratiom_rank = """
        DROP TABLE IF EXISTS ranks_{next};
        CREATE TABLE ranks_{next} AS
        SELECT Graph.dst AS usr, SUM(r.Rank/outdegrees.out) AS Rank
        FROM ranks_{curr} AS r, Graph, outdegrees
        WHERE r.usr = Graph.src AND r.usr = outdegrees.usr
        GROUP BY Graph.dst;
        """.format(
                curr=i,
                next=i+1
            )
        runq(iteratiom_rank, db)
    


    # Return results with schema (username, page_rank_score)
    final_query = """
    SELECT ranks_{last}.usr AS username, Rank as page_rank_score
    FROM ranks_{last}
    ORDER BY page_rank_score DESC LIMIT 20;
    """.format(last=n_iter)
    return runq(final_query, db)

result = page_rank(20, db)
assert len(result.columns)== 2, "The result should contain two columns: username, page_rank_score!"
result
...
q8 = """
WITH RECURSIVE ranks(usr, rank, iteration) AS(
  SELECT DISTINCT src as usr, 1/CAST((SELECT COUNT(*) FROM ((SELECT DISTINCT src AS usr FROM Graph) UNION (SELECT DISTINCT dst AS usr FROM Graph))) AS Double) AS Rank, 0 AS iteration
  FROM Graph
  UNION
  SELECT Graph.dst AS usr, SUM(ranks.Rank/outdegrees.out) AS Rank, (SELECT MAX(iteration) FROM ranks) + 1 AS iteration
  FROM ranks, Graph, (SELECT src AS usr, COUNT(*) AS out from Graph GROUP BY src) AS outdegrees
  WHERE ranks.usr = Graph.src AND ranks.usr = outdegrees.usr AND ranks.iteration <= 19 AND ranks.iteration = (SELECT MAX(iteration) FROM ranks)
  GROUP BY Graph.dst
)
SELECT usr AS username, Rank as page_rank_score FROM ranks WHERE iteration == 20 ORDER BY page_rank_score DESC LIMIT 20;
"""
result = runq(q8)
assert len(result.columns)== 2, "The result should contain two columns: username, page_rank_score!"
result
...
2.   You don't use any modules beyond [Python Standard Library](https://docs.python.org/3/library/). (besides duckdb)
3.   You don't run any operating system shell command (prepended by exclamation point) besides the setup codes we provided.
4.   You don't use SQL magic (%SQL, %%SQL)
...
3.   Name the file "project2.py" (make sure the name is exactlt the same) and download the file to your local storage.
...
Upload the "project2.py" (make sure the name is exactly the same name) to Gradescope HW0. We have tests to check that your .py file is executable. Make sure you pass the first two tests.
